            ...
            for (let i = 0; i < list.items.length; i++) {
                ...
                if (!list.loose) {
                    ...
                    const hasMultipleLineBreaks =
                        spacers.length > 0 && spacers.some((t) => this.rules.other.anyLine.test(t.raw));
    ...
    emStrong(src, maskedSrc, prevChar = '') {
        ...
        if (!nextChar || !prevChar || this.rules.inline.punctuation.exec(prevChar)) {
            ...
            let rDelim,
                ...
                midDelimTotal = 0;
            const endReg =
                match[0][0] === '*' ? this.rules.inline.emStrongRDelimAst : this.rules.inline.emStrongRDelimUnd;
            endReg.lastIndex = 0;
    ...
    codespan(src) {
        ...
        if (cap) {
            ...
            const hasSpaceCharsOnBothEnds =
                this.rules.other.startingSpaceChar.test(text) && this.rules.other.endingSpaceChar.test(text);
        ...
        const parse = (src, options) => {
            ...
            if (opt.async) {
                return Promise.resolve(opt.hooks ? opt.hooks.preprocess(src) : src)
                    ...
                    .then((tokens) =>
                        opt.walkTokens
                            ? Promise.all(this.walkTokens(tokens, opt.walkTokens)).then(() => tokens)
                            : tokens
                    )
